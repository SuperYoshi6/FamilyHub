...
const ListsPage: React.FC<ListsPageProps> = ({
    ...
    const getTabContainerClass = () => "bg-gray-100 dark:bg-gray-800 p-1 rounded-xl w-full shadow-inner flex";
    ...
    const handleAddRecipeByLink = async (e: React.FormEvent) => {
        ...
        setTimeout(() => {
            const mockRecipe: Recipe = {
                ...
                description: "1. Die Pasta in reichlich Salzwasser al dente kochen, abgießen und abkühlen lassen.\n2. In der Zwischenzeit die Tomaten halbieren, die Gurke würfeln und den Feta zerbröseln.\n3. Alles in eine große Schüssel geben, Oliven hinzufügen.\n4. Ein Dressing aus Olivenöl, Balsamico, Salz und Pfeffer anrühren und unter den Salat heben.\n5. Vor dem Servieren mindestens 30 Minuten ziehen lassen.",
                        ...
                        <div key={recipe.id} className={`rounded-xl overflow-hidden border flex flex-col bg-white border-gray-100 dark:bg-gray-800 dark:border-gray-700`}>
                            {recipe.image && <img src={recipe.image} className="w-full h-32 object-cover cursor-pointer" onClick={() => {
                                if (isExpanded) {
                                    ...
                                    setSelectedIngredients(new Set(recipe.ingredients)); // Select all by default
                                }
                            }} />}
                            <div className="p-3 flex-1 flex flex-col">
                                ...
                                {isExpanded && (
                                    <div className="mt-2 mb-4 animate-fade-in border-t border-gray-100 dark:border-gray-700 pt-2">
                                        {recipe.description && (
                                            <div className="mb-3">
                                                <h5 className="text-[10px] font-bold uppercase text-gray-400 mb-1">Zubereitung</h5>
                                                <p className="text-xs text-gray-600 dark:text-gray-300 whitespace-pre-line">{recipe.description}</p>
                                            </div>
                                        )}
                                ...
                                <div className="mt-auto flex justify-between items-center pt-2">
                                    {isExpanded ? (
                                        ...
                                        >
                                            <ShoppingCart size={14} /> Markierte auf Liste
                                        </button>
                                    ) : (
                                        <button
            ...
            <div className="px-4 mt-2">
                <div className={getTabContainerClass()}>
                    {tabs.map((tab) => {
                        ...
                        let btnClass = isActive ? "bg-white dark:bg-gray-700 shadow-sm text-gray-900 dark:text-white" : "text-gray-500 dark:text-gray-400";
