...
const SettingsPage: React.FC<SettingsPageProps> = ({
    ...
    const handleFullSync = async () => {
        if (!events || events.length === 0) {
            alert("Keine Termine zum Synchronisieren gefunden.");
            return;
        }
        ...
        try {
            ...
            await NativeCalendarService.syncAllToNative(events);
            alert(`${events.length} Termine wurden erfolgreich mit deinem Geräte-Kalender synchronisiert.`);
        } catch (err) {
            console.error(err);
            alert("Fehler bei der Synchronisation mit dem System-Kalender.");
        } finally {
            ...
            <div className="flex flex-col min-h-full pb-20">
                ...
                <div className={`z-40 transition-all duration-500 overflow-hidden ${liquidGlass ? 'backdrop-blur-xl bg-white/10 shadow-[0_8px_24px_rgba(31,38,135,0.05)]' : 'bg-white dark:bg-gray-900 shadow-[0_1px_0_rgba(0,0,0,0.03)]'}`}>
                    <div className="px-4 py-4 flex items-center">
                        ...
                        <span className={`font-black text-xl tracking-tight ${liquidGlass ? 'text-slate-900 dark:text-white' : 'text-gray-800 dark:text-white'}`}>{t('settings.title', lang)}</span>
                    </div>
                </div>
                ...
                <div className="flex-1 p-6 space-y-8 max-w-md mx-auto w-full">
                    ...
                    <section className="space-y-6">
                        ...
                        <div className={`p-4 rounded-2xl shadow-sm space-y-4 ${sectionBgClass}`}>
                            ...
                            {isAdmin && currentUser.id === 'admin' ? null : isAdmin && (
                            ...
                            {!messageSent ? (
                                <div className="space-y-4">
                                    <div>
                                        ...
                                        >
                                            <option value="" disabled>Wähle jemanden aus...</option>
                            ...
                            <div className="flex-1 overflow-y-auto p-6 custom-scrollbar space-y-6">
                                ...
                                >
                                    <div>
                                        ...
                                        <p className="text-gray-400 text-sm">Aktualisierte Version laden.</p>
                                    </div>
